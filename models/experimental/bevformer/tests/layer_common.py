...
def build_layer_fixture(device, config, bev_size, batch_size: int, dtype=ttnn.bfloat16, lidar2img=None) -> LayerFixture:
    ...
    Geometry stays on ``forward`` here: the encoder still generates reference
    points and camera projections once, then passes them into every layer.
    """
    ...
    # ``TTBEVFormerLayer.forward`` still slices ``reference_points_3d`` as a
    # torch tensor and uploads the BEV slice itself, matching the encoder.
    tt_inputs = {
