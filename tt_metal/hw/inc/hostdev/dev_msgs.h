...
struct kernel_config_msg_t {
    ...
    // Was ... + 4; those bytes now hold reload_table_addr, so the message keeps its size and
    // alignment on every arch.
    volatile uint8_t pad3[1 + ((1 - MaxProcessorsPerCoreType % 2) * 10)];  // CODEGEN:skip
