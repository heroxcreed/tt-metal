...
class ComputePipeline:
    ...
    @staticmethod
    def _output_collector(node: PackNode, tilized: bool):
        if node.pack_l1_accumulation == L1Accumulation.Yes:
            return L1AccumOutputTiles(node.output)
        if node.packer.untilizes_l1_output:
            return UntilizePackOutput(node.output)
        if tilized:
            return TilizedOutputTiles(node.output)
        return OutputTiles(node.output)

    def _produces_tilized_l1(self) -> bool:
        """True when a tilize unpacker makes the output a tile-concatenated L1 result."""
        return any(
            isinstance(node, FpuNode)
            and node.unpacker is not None
            and node.unpacker.produces_tilized_l1
            for node in self.math_nodes
        )
    ...
    def _per_call_golden(
        ...
        tilized = self._produces_tilized_l1()
        outputs = {
            id(node): self._output_collector(node, tilized) for node in pack_nodes
        }
                ...
                for node in self.math_nodes:
                    ...
                    if isinstance(node, SfpuNode):
                        for call in planned.plan(node, "sfpu").calls(bank):
                            node.sfpu.golden(call, dest, node, operation, config)
                        continue
