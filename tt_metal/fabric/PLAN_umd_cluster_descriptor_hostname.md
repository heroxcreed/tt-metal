...
1. `TT_CLUSTER_ID` environment variable, if set and non-empty.
2. otherwise POSIX `gethostname()`.

The env var is what makes the field usable in containers and VMs: the operator (or the launcher) sets `TT_CLUSTER_ID` to the id of the physical accelerator group, and UMD stamps that instead of the meaningless in-guest hostname.
...
|------|------------------------|--------------|
| Live silicon | `TopologyDiscovery::fill_cluster_descriptor_info` stamps `TT_CLUSTER_ID` else `gethostname()` | metal discovery, `serialize()` |
...
| Container / VM | operator sets `TT_CLUSTER_ID`; UMD stamps it verbatim | same as live silicon |
...
| `TT_CLUSTER_ID` unset | `gethostname()`, i.e. exactly today's effective value on bare metal. |
...
public:
    ...
    // (or $TT_CLUSTER_ID); semantically the accelerator group, not a machine name.
...
Do **not** add a second metal wrapper type. `tt::umd::Cluster::get_cluster_description()->get_cluster_id()` is the query. Nanobind: bind `get_cluster_id` / `set_cluster_id` next to the other `ClusterDescriptor` getters (`py_api_topology_discovery.cpp`).

`create_mock_cluster(...)` stays id-less (`nullopt`) unless a new optional argument is passed. Simulation / unit mocks do not need a galaxy id, and they must **not** pick one up from `TT_CLUSTER_ID` (§4.1).
...
cluster_id:
  ...
  description: >
    ...
    and the fabric topology solver key on), or the value of $TT_CLUSTER_ID when
...
**Parse** (`create_from_yaml_content`): if `yaml["cluster_id"]` is defined, `set_cluster_id(as<string>)`. Missing → leave `nullopt`. Present but invalid → throw (same as a bad `arch` key).
...
| `TopologyDiscovery::fill_cluster_descriptor_info` | `set_cluster_id(local_cluster_id())` after the chip tables are filled |
| `create_mock_cluster` | leave unset unless an optional `cluster_id` argument is added |

### 4.1 `local_cluster_id()` — the fill rule

Small helper next to the descriptor in UMD:

```
local_cluster_id():
    if getenv("TT_CLUSTER_ID") is set and non-empty after trim:
        validate it (§5); fatal on invalid
        return it
    return gethostname()          // POSIX, raw result, no FQDN strip in UMD
```

- **Empty / whitespace-only `TT_CLUSTER_ID` counts as unset** — an exported-but-empty var is a launcher accident, not a request for an empty id.
- **Invalid `TT_CLUSTER_ID` is fatal, not a fallback to `gethostname()`.** Someone set it on purpose; silently substituting a container hostname yields a wrong-but-plausible topology, which is the exact failure this field exists to prevent. Fail loudly.
...
- `create_from_yaml` does **not** apply `TT_CLUSTER_ID` when the YAML lacks the key. A mock YAML describes *some other* machine; the env var describes *this* one. Substituting would relabel every mock descriptor as the local host and collapse a multi-host mock into one node.
...
If the key is present, `set_cluster_id` is called, or `TT_CLUSTER_ID` is set:
...
Metal does **not** open the YAML itself to peek at `cluster_id:`, and metal does **not** read `TT_CLUSTER_ID`. Always go through `ClusterDescriptor::get_cluster_id()` — one reader of the env var, in UMD (§4.1).
...
- `set_cluster_id("")` / whitespace / 129+ chars / `foo/bar` throw; exactly 128 chars is accepted.
- `create_constrained_cluster_descriptor` and `apply_chip_id_remapping` copy the id.
- `local_cluster_id()` with `TT_CLUSTER_ID=tt-vm-host-7` → returns it; unset → returns `gethostname()`; `TT_CLUSTER_ID=""` / `"   "` → treated as unset; `TT_CLUSTER_ID="a/b"` or 129+ chars → fatal (**not** a silent fallback).
- **Env var does not leak into parsing:** with `TT_CLUSTER_ID` set, a YAML *without* the key still yields `nullopt`, and a YAML *with* `cluster_id: other-host` still yields `other-host`.
- Nanobind: `ClusterDescriptor.create_from_yaml_content(...).get_cluster_id()`.
- Hardware (optional, `TopologyDiscovery`): after `discover`, `get_cluster_id()` equals `gethostname()` with no env var, and equals `TT_CLUSTER_ID` when it is set.
...
- `device/api/umd/device/cluster_descriptor.hpp` — getter, setter, `cluster_id_`
...
- `device/…/local_cluster_id.{hpp,cpp}` (or the existing small-utils home) — `TT_CLUSTER_ID` → `gethostname()` fallback + validation + the one log line
...
- UMD docs / README env-var table — document `TT_CLUSTER_ID` (containers and VMs need it; bare metal does not)
...
1. **UMD PR** — optional `cluster_id` field + query + `TT_CLUSTER_ID`/`gethostname()` stamp + tests. Old YAMLs still load (back-compat).
...
4. **TODO / tt-cluster-descriptors** — write `cluster_id:` on **all** cluster descriptor YAMLs (§7). Can land incrementally; pin when a batch is ready.
5. Drop the aisle-token fallback once FSD-paired BH files are filled.
6. Container / VM enablement: set `TT_CLUSTER_ID` in the launchers and images that run fabric workloads (no code change; §11 tracks the rest).
...
- Reading `TT_CLUSTER_ID` anywhere except `local_cluster_id()` (not in metal, not in YAML parsing, not in `create_mock_cluster`)
