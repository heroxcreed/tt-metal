...
- [ ] `ClusterDescriptor::get_cluster_id()` / `set_cluster_id()`; parse if present, `nullopt` if missing; reject empty / illegal strings.
...
- [ ] `TopologyDiscovery::fill_cluster_descriptor_info` stamps `local_cluster_id()` = `$TT_CLUSTER_ID` else `gethostname()`. Env var read in that one helper only — not in YAML parsing, not in `create_mock_cluster`, not in metal.
- [ ] Nanobind get/set.
...
- [ ] Metal does **not** read `TT_CLUSTER_ID` itself — only `ClusterDescriptor::get_cluster_id()`.
